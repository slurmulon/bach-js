...
#### `.unitize(duration, lens)`

Converts a step-based duration value from one unit to anther using a lens object.

Identical to `.cast` but uses `step` instead of `ms` as the underlying unit.

Since `step` is normalized as an integer, this method helps avoid floating-point precision loss that can occur when working with milliseconds in `cast`.
...
#### `.elapsed(duration, lens)`

Converts a duration value and determines the delta from `min`.
...
```js
durations.elapsed(2) // 2
durations.elapsed(2, { as: 'ms' }) // 2000
durations.elapsed(3, { as: 'ms', min: 1.5 }) // 1500
```
...
#### `.interpolate(ratio, lens)`

Converts a ratio value (0 - 1) into a step unit within a range.

 - `min`: Lower bound of the range using `is` unit (default: `0`)
 - `max`: Lower bound of the range using `is` unit (default: `0`)

```js
durations.interpolate(0.5, { min: 2, max: 6 }) // 4
```
